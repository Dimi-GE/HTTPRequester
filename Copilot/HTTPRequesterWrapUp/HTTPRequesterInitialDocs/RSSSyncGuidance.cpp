 ...
 */
bool CreateRemoteManifest(const FString& TempFolder)
{
    FString ExtractedPath = TempFolder + TEXT("extracted/");
    
    // Find the actual repository folder (GitHub creates a folder named repo-branch)
    TArray<FString> FoundDirs;
    IFileManager::Get().FindFiles(FoundDirs, *ExtractedPath, TEXT("*"), false, true);
    
    if (FoundDirs.Num() == 0)
    {
        UE_LOG(LogTemp, Error, TEXT("RSSSync: No extracted repository folder found"));
        return false;
    }
    
    // Use the first directory found (should be the repo folder)
    FString RepoPath = ExtractedPath + FoundDirs[0] + TEXT("/");
    ...
    FString ManifestPath = TempFolder + TEXT("RemoteRSSManifest.json");
...
bool UpdateManifestsAfterChanges(const FString& TempFolder)
{
    ...
    if (IFileManager::Get().DirectoryExists(*UpdatedPath))
    {
        try
        {
            ...
            bRemoteSuccess = CreateRemoteManifest(TempFolder);
                   ...
                   bool bExecuteChanges = false, const FString& CommitMessage = TEXT("RSS Sync Update"))
{
    ...
    DownloadBranchFromGitHub(RepoOwner, RepoName, BranchName, AccessToken, TempFolder,
        [=](bool bDownloadSuccess)
        {
            ...
            if (!CreateRemoteManifest(TempFolder))
            {
